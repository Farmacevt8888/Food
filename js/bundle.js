...
function cards() {
  ...
  class MenuCard {
    constructor(src, alt, title, descr, price, paretnSelector) {
      this.src = src;
      this.alt = alt;
      this.title = title;
      this.descr = descr;
    ...
    render() {
      const element = document.createElement('div'); // устанавливаем класс по умолчанию
...
function slider(_ref) {
  ...
  let slideIndex = 1;
  let offset = 0; //----------slider-couruselle-------------
...
function tabs(tabsSelector, tabsContentSelector, tabsParentSelector, activeClass) {
  ...
  let tabs = document.querySelectorAll(tabsSelector),
      ...
      tabsParent = document.querySelector(tabsParentSelector); // функция скрывающая ненужные табы
  ...
  hideTabsContent();
  showTabContent(); // делегирование событий с обработчиком клика
...
async function getResource(url) {
  let res = await fetch(url); //устанавливаем вручную показ ошибки, если что-то пойдет не так
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_timer__WEBPACK_IMPORTED_MODULE_3__["default"])('.timer', '2022-10-01');
